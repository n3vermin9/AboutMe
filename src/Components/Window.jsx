...
function Window({ title, onClose, game}) {
  ...
  let casinoArr = [
    {id:1, name: 'readme.txt', link: 'https://upload.wikimedia.org/wikipedia/commons/2/23/Text-txt.svg', content:`A self-contained, front-end casino simulator built with pure client-side technologies. This project delivers the full experience of six popular casino games in the browser, with no backend server required.`},
  ...
  return (
    <div className={`fade-in absolute m-auto top-0 bottom-0 right-0 left-0
    ${title === 'Purble place' ? 'h-[495px] w-[750px]' : 'h-[400px] w-[500px]'} rounded-lg border-2 border-white
      ...
      <div className="flex justify-between items-center mb-2">
        ...
        {title === 'Purble place' ? null:
          ...
          {currentSection === 'nvrCasino' && 
            <button className="border-2 flex items-center justify-center absolute left-2 top-[7px] rounded-full
              bg-gradient-to-t from-[#36d7ff7a] to-[#1f9e50b6] z-50
              w-7 h-7 text-white text-xl" onClick={goBack}>←</button>}
        </span>}
        <button 
        ...
        <div className='flex absolute gap-8 flex-wrap top-[88px]'>
          {title === 'Skills' ? skillsArr.map(skill => (
            <Picture key={skill.id} image={skill.link}
              title={skill.name}
              onClick={() => handlePictureClick(skill.link, skill.name)}/>
          )) : null}
          {title === 'About me' ? aboutArr.map(skill => (
            <Picture key={skill.id} image={skill.link}
             title={skill.name}
             onClick={() => handlePictureClick(skill.link, skill.name, skill.content)}/>
          )) : null}
          <div>
              ...
              <div className='flex w-[465px] absolute gap-8 flex-wrap'>
                {
                  ...
                  <a href="https://n3vermin9.github.io/casino3/index.html" target="_blank" rel="noopener noreferrer">
                    <Picture key={Date.now()} image={'https://images.sftcdn.net/images/t_app-icon-m/p/c566a854-96d0-11e6-8864-00163ed833e7/275216958/internet-explorer-11-windows-7-logo.png'}
                    title='live_example.html'/>
                  </a>
              </div>
            }
            {currentSection !== 'nvrCasino' && title === 'Projects' && (
              projectsArr.map(project => (
                <Folder key={project.id} onClick={() => handleFolderClick(project.name)} name={project.name} />
              ))
            )}
          </div>
        </div>
      ...
      {modalOpen && selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-md max-w-xl w-full relative border-2 ">
            <span className='absolute top-0 left-0 w-full h-[23px] text-white pl-2 text-[12px] flex items-center bg-gradient-to-t from-[#58866c60] to-[#01d6a43d] border-b border-white rounded-t-[4px]'>
        <p className='absolute top-0 right-0 left-0 m-auto flex items-center
        justify-center text-black'>{selectedImage.name}</p>
        </span>
        <button 
          ...
          onClick={closeModal}>
            ...
            />:
             <p
              style={{
                backgroundImage: 'repeating-linear-gradient(0deg, rgba(0,0,0,.6) 0, rgba(255,255,255,0.125) 1px, transparent 1px, transparent 40px)',
              }}
              className="pl-5 pr-5 bg-gradient-to-t from-[#d3a50f7a] to-[#e69a44b6] w-full h-auto mt-[23px] bg-[length:24px_24px]">{selectedImage.content}</p>
            }
          </div>
        </div>
      )}
